...
import pl.koziarz.specificationexample.domain.specification.ChildLikesToySpecification;
...
public class KindergartenApp {
	...
	public static void main(String[] args) {
		...
		ISpecification<Child> spec_red_toy = new ChildLikesSpecifiedToySpecification(new ToyColorSpecification("Red"));
		 ...
		 */
		ISpecification<Child> spec_not_red_toy = new ChildLikesSpecifiedToySpecification(new ToyColorSpecification("Red").not());
		...
		ISpecification<Child> spec_johny_likes_red_toys = spec_johny.and(spec_red_toy);
		 ...
		 */
		ISpecification<Child> spec_schoolchildren = new SchoolChildrenSpecification(); 
		System.out.println("\nSchool Children");
		printSpecified(children, spec_schoolchildren);
                ...
                System.out.println("\nNew Specification");
                AbstractSpecification<Child> spec2 = new AndSpecification(spec, spec_schoolchildren);
		printSpecified(children, spec2.and(spec_johny).or(new ChildLikesToySpecification(ferrari)));
                
                //
                
	}
	
	static <T> void printSpecified(Set<T> set, ISpecification<T> spec) {
		for(T t : set) {
